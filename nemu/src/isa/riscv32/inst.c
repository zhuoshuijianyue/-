...
enum {
  TYPE_I, TYPE_U, TYPE_S,TYPE_R,TYPE_B,TYPE_J,TYPE_ISP,
  TYPE_N// none
};
...
#define immISP() do { *imm = BITS(i, 24, 20); } while(0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    ...
    case TYPE_ISP:src1R();         immISP();break;
    
  }
}
word_t mulh(word_t a,word_t b) {
	word_t upper=(long long)((long)(signed)a*(long)(signed)b)>>32;
	return upper;
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  //   31   25 |26 22| 21 17| 18 16| 15  11    7 6   0
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) =  imm);
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd)=s->pc+4;s->dnpc=s->pc+imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd)=s->pc+4;s->dnpc=((src1+imm)/2*2));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , ISP, R(rd)=src1<<imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , ISP, R(rd)=src1>>imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , ISP, R(rd)=(signed)src1>>imm);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd)=((signed)src1)>>(src2%32));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd)=(((signed)src1)%src2)); //may nor right
